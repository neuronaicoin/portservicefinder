...
export default function Home() {
  ...
  const nav = (k) => k === 'agent' ? 'Ship Agent' : k === 'chandler' ? 'Shipchandler' : 'Marine Service';

  function search(c, p, s, m) {
    if (!c || !p) { setDone(false); return; }
    const {r, fb: f} = runSearch(c, p, s, m) as {r: Provider[], fb: boolean};
  ...
  const sel = {background:'rgba(8,16,10,.85)',border:'1px solid rgba(200,168,75,.3)',color:'#f5f0e8',padding:'13px 16px',fontSize:14,width:'100%',outline:'none'} as React.CSSProperties;
      ...
      <div style={{background:'#08100a',color:'#f5f0e8',fontFamily:"'Outfit',sans-serif",fontWeight:300,minHeight:'100vh'}}>
        ...
        <section style={{background:'#08100a',padding:'60px 56px',textAlign:'center'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:1,background:'rgba(200,168,75,.12)',maxWidth:900,margin:'0 auto'}}>
            {[['80+','Countries','with active providers'],['1,000+','Ports','in our database'],['37+','Providers','verified and growing'],['$0','Search Fee','forever free']].map(([v,l,s])=>(
        ...
        <section id="how" style={{padding:'100px 56px',background:'#0c1610',borderTop:'1px solid rgba(200,168,75,.15)'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:1,background:'rgba(200,168,75,.15)'}}>
            {[{n:'01',ico:'&#127757;',t:'Select Country & Port',d:'Choose your destination from our global list. Completely free.'},
              ...
              {n:'03',ico:'&#128225;',t:'Smart Country Fallback',d:'No provider at your port? We show others in the same country.'}].map(s=>(
              <div key={s.n} className="step" style={{background:'#111c13',padding:'42px 34px',position:'relative',overflow:'hidden',transition:'background .4s'}}>
                ...
                <div style={{fontSize:28,marginBottom:16}} dangerouslySetInnerHTML={{__html:s.ico}}/>
        ...
        <section id="pricing" style={{padding:'100px 56px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:18,maxWidth:760,margin:'0 auto'}}>
            {[{name:'Monthly',amt:'$149',per:'/ month',yr:'Billed monthly, cancel anytime',badge:null,items:['Listed at all your ports','Full company profile','Phone, email and WhatsApp','Verified provider badge','Performance dashboard']},
              {name:'Annual',amt:'$1,200',per:'/ year',yr:'Equivalent to $100 per month',badge:'Save $588',items:['Everything in Monthly plan','$588 saved vs monthly','Priority placement in results','Verified provider badge','Priority email support']}].map(tier=>(
        ...
        <section style={{padding:'80px 56px',textAlign:'center',background:'#0c1610',borderTop:'1px solid rgba(200,168,75,.15)'}}>
          ...
          <p style={{fontSize:15,color:'#b0c0a4',maxWidth:460,margin:'0 auto 36px',lineHeight:1.75}}>List on PortServiceFinder for <strong style={g}>$149/month</strong> or <strong style={g}>$1,200/year</strong>.</p>
          <div style={{display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap'}}>
            ...
            <button onClick={()=>document.getElementById('search')?.scrollIntoView({behavior:'smooth'})} style={{background:'transparent',color:'#f5f0e8',border:'1px solid rgba(200,168,75,.3)',padding:'13px 28px',fontFamily:rj,fontSize:15,letterSpacing:'2px',textTransform:'uppercase',fontWeight:600,cursor:'pointer'}}>Search Free</button>
          </div>
        </section>
        ...
        {detail&&(<div style={{position:'fixed',inset:0,background:'rgba(8,16,10,.95)',backdropFilter:'blur(16px)',zIndex:550,display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'40px 20px',overflowY:'auto'}} onClick={e=>{if(e.target===e.currentTarget)setDetail(null);}}>
          <div style={{background:'#0c1610',border:'1px solid rgba(200,168,75,.3)',width:'100%',maxWidth:720,margin:'auto'}}>
            <div style={{padding:'30px 36px 22px',borderBottom:'1px solid rgba(200,168,75,.2)',display:'flex',gap:18,alignItems:'flex-start'}}>
              ...
              <button onClick={()=>setDetail(null)} style={{background:'none',border:'none',color:'#7a8a72',fontSize:20,cursor:'pointer',flexShrink:0}}>X</button>
            </div>
            <div style={{padding:'26px 36px 32px'}}>
              ...
              <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
                <a href={`tel:${detail.phone.replace(/\s/g,'')}`} style={{flex:1,minWidth:140,padding:13,background:'#c8a84b',color:'#08100a',textDecoration:'none',fontFamily:rj,fontSize:13,letterSpacing:'1.5px',textTransform:'uppercase',fontWeight:700,textAlign:'center',display:'flex',alignItems:'center',justifyContent:'center'}}>Call</a>
                <a href={`mailto:${detail.email}`} style={{flex:1,minWidth:140,padding:13,background:'#c8a84b',color:'#08100a',textDecoration:'none',fontFamily:rj,fontSize:13,letterSpacing:'1.5px',textTransform:'uppercase',fontWeight:700,textAlign:'center',display:'flex',alignItems:'center',justifyContent:'center'}}>Email</a>
        ...
        {modal&&(<div style={{position:'fixed',inset:0,background:'rgba(8,16,10,.95)',backdropFilter:'blur(16px)',zIndex:500,display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'40px 20px',overflowY:'auto'}} onClick={e=>{if(e.target===e.currentTarget)setModal(false);}}>
          <div style={{background:'#0c1610',border:'1px solid rgba(200,168,75,.3)',width:'100%',maxWidth:740,margin:'auto'}}>
            <div style={{padding:'26px 34px 18px',borderBottom:'1px solid rgba(200,168,75,.2)',display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              <div><h2 style={{fontFamily:lb,fontSize:24,fontWeight:700,marginBottom:4}}>{tab==='login'?'Provider Sign In':'List Your Business'}</h2></div>
              <button onClick={()=>setModal(false)} style={{background:'none',border:'none',color:'#7a8a72',fontSize:20,cursor:'pointer'}}>X</button>
            </div>
              ...
              {tab==='register'?(
                <div>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:20}}>
                    {[{type:'agent',ico:'&#127970;',name:'Ship Agent'},{type:'chandler',ico:'&#9875;',name:'Shipchandler'},{type:'service',ico:'&#128295;',name:'Marine Service'}].map(s=>(<div key={s.type} onClick={()=>setSeg(s.type)} style={{border:`1px solid ${seg===s.type?'#c8a84b':'rgba(200,168,75,.2)'}`,padding:'14px 10px',textAlign:'center',cursor:'pointer',background:seg===s.type?'rgba(200,168,75,.1)':'transparent'}}><div style={{fontSize:20,marginBottom:5}} dangerouslySetInnerHTML={{__html:s.ico}}/><div style={{fontFamily:rj,fontSize:12,letterSpacing:1,textTransform:'uppercase',fontWeight:700}}>{s.name}</div></div>))}
                  </div>
                  ...
                  <div style={{marginBottom:16}}><label style={{display:'block',fontFamily:rj,fontSize:11,letterSpacing:'1.5px',textTransform:'uppercase',color:'#7a8a72',marginBottom:4,fontWeight:600}}>Company Bio</label><textarea maxLength={500} placeholder="Brief description..." style={{background:'rgba(8,16,10,.7)',border:'1px solid rgba(200,168,75,.25)',color:'#f5f0e8',padding:'10px 13px',fontSize:14,width:'100%',resize:'vertical',minHeight:80}}/></div>
        ...
        {payModal&&(<div style={{position:'fixed',inset:0,background:'rgba(8,16,10,.96)',backdropFilter:'blur(20px)',zIndex:600,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px',overflowY:'auto'}} onClick={e=>{if(e.target===e.currentTarget)setPayModal(false);}}>
          <div style={{background:'#0c1610',border:'1px solid rgba(200,168,75,.3)',width:'100%',maxWidth:620,padding:40,margin:'auto',position:'relative'}}>
            <button onClick={()=>setPayModal(false)} style={{position:'absolute',top:18,right:18,background:'none',border:'none',color:'#7a8a72',fontSize:20,cursor:'pointer'}}>X</button>
            <h2 style={{fontFamily:lb,fontSize:26,fontWeight:700,marginBottom:6}}>Choose Your <em style={g}>Plan</em></h2>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:26}}>
              {[{id:'monthly',label:'Monthly',price:'$149',period:'per month',note:'Billed monthly.',badge:null},{id:'yearly',label:'Annual',price:'$1,200',period:'per year',note:'$100/month equivalent.',badge:'Save $588'}].map(p=>(
            ...
            <button onClick={()=>{setPayModal(false);alert('Welcome to PortServiceFinder!');}} style={{width:'100%',padding:14,background:'#c8a84b',border:'none',color:'#08100a',fontFamily:rj,fontSize:14,letterSpacing:'2px',textTransform:'uppercase',fontWeight:700,cursor:'pointer',marginTop:18}}>
              Pay {plan==='yearly'?'$1,200':'$149'} and Activate
            </button>
          </div>
        </div>)}
...
function FI({l,p,t='text'}:{l:string;p:string;t?:string}) {
  return (
    <div style={{marginBottom:10}}>
      ...
      <input type={t} placeholder={p} style={{background:'rgba(8,16,10,.7)',border:'1px solid rgba(200,168,75,.25)',color:'#f5f0e8',padding:'10px 13px',fontSize:14,width:'100%'}}/>
    </div>
  );
}
