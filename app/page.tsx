...
export default function Home() {
  ...
  const nav2 = (k: string) => k === 'agent' ? 'Ship Agent' : k === 'chandler' ? 'Shipchandler' : 'Marine Service';
  ...
  const sel = {background:'rgba(8,16,10,.9)',border:'1px solid rgba(200,168,75,.3)',color:'#f5f0e8',padding:'11px 14px',fontSize:14,width:'100%',outline:'none'} as React.CSSProperties;
  const lbl = {display:'block',fontFamily:rj,fontSize:11,fontWeight:700,letterSpacing:'1.5px',textTransform:'uppercase' as const,color:'#c8a84b',marginBottom:5};
      ...
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        @media(max-width:768px){
          ...
          .nav-btn-signin{display:none!important;}
          .nav-btn-list{font-size:11px!important;padding:7px 12px!important;letter-spacing:1px!important;}
          .hero-pad{padding:90px 24px 60px!important;}
          .search-box{padding:24px 20px!important;}
          .search-grid{grid-template-columns:1fr!important;}
          .visuals-pad{padding:40px 24px!important;}
          .stats-grid{grid-template-columns:repeat(2,1fr)!important;}
          .section-pad{padding:60px 24px!important;}
          .steps-grid{grid-template-columns:1fr!important;}
          .tiers-grid{grid-template-columns:1fr!important;}
          .footer-grid{grid-template-columns:1fr!important;}
          .footer-pad{padding:40px 24px!important;}
          .cta-pad{padding:60px 24px!important;}
          .detail-contact{grid-template-columns:1fr!important;}
          .modal-pad{padding:20px!important;}
        }
      `}</style>
      <div style={{background:'#08100a',color:'#f5f0e8',fontFamily:"'Outfit',sans-serif",fontWeight:300,minHeight:'100vh'}}>

        {/* NAV */}
        <nav style={{position:'fixed',top:0,width:'100%',zIndex:300,height:66,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 32px',background:'rgba(8,16,10,.97)',backdropFilter:'blur(20px)',borderBottom:'1px solid rgba(200,168,75,.2)'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:16}}>
            ...
            <button className="nav-btn-signin" style={{background:'transparent',border:'1px solid rgba(200,168,75,.4)',color:'#c8a84b',padding:'8px 18px',fontFamily:rj,fontSize:12,letterSpacing:'1.5px',textTransform:'uppercase',fontWeight:700,cursor:'pointer'}} onClick={()=>{setTab('login');setModal(true);}}>Sign In</button>
            <button className="nav-btn-list" style={{background:'#c8a84b',color:'#08100a',border:'none',padding:'8px 18px',fontFamily:rj,fontSize:12,letterSpacing:'1.5px',textTransform:'uppercase',fontWeight:700,cursor:'pointer',whiteSpace:'nowrap'}} onClick={()=>{setTab('register');setModal(true);}}>List Your Business</button>
          </div>
        </nav>

        {/* HERO */}
        <section className="hero-pad" style={{minHeight:'100vh',paddingTop:120,paddingBottom:80,paddingLeft:56,paddingRight:56,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',textAlign:'center',gap:36}}>
          <div className="a1" style={{fontFamily:rj,fontSize:11,letterSpacing:'4px',textTransform:'uppercase',color:'#c8a84b',display:'flex',alignItems:'center',gap:14,justifyContent:'center'}}>
            <span style={{width:36,height:1,background:'#c8a84b',display:'inline-block',opacity:.6}}/>
            Global Maritime Services Directory
            <span style={{width:36,height:1,background:'#c8a84b',display:'inline-block',opacity:.6}}/>
          </div>
          <h1 className="a2" style={{fontFamily:lb,fontSize:'clamp(42px,6vw,80px)',fontWeight:700,lineHeight:1.02,letterSpacing:-2,maxWidth:880}}>
            Every Port. Every <em style={g}>Service.</em><br/>One Platform.
          </h1>
          <p className="a3" style={{fontSize:16,lineHeight:1.8,color:'#b0c0a4',maxWidth:500}}>
          ...
          <div className="a4" style={{display:'flex',gap:20,flexWrap:'wrap',justifyContent:'center'}}>
            {[['160+','Countries'],['1,000+','Ports'],['22','Categories']].map(([n,l])=>(
              <span key={l} style={{fontFamily:rj,fontSize:13,color:'#7a8a72',fontWeight:600}}><strong style={g}>{n}</strong> {l}</span>
            ))}
          </div>

          {/* SEARCH BOX — inside hero */}
          <div className="a4 search-box" style={{width:'100%',maxWidth:840,background:'rgba(10,20,14,.95)',border:'1px solid rgba(200,168,75,.3)',backdropFilter:'blur(20px)',padding:'32px 36px',marginTop:8}}>
            <div className="search-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr auto',gap:12,alignItems:'flex-end',marginBottom:0}}>
              ...
              <button style={{background:'#c8a84b',color:'#08100a',border:'none',padding:'11px 24px',fontFamily:rj,fontSize:14,letterSpacing:'2px',textTransform:'uppercase',fontWeight:700,cursor:'pointer',height:44,whiteSpace:'nowrap'}} onClick={()=>search(country,port,svcType,ms)}>Search</button>
            </div>
            {svcType==='service'&&(
              <div style={{marginTop:14,padding:'14px 16px',background:'rgba(200,168,75,.04)',border:'1px solid rgba(200,168,75,.15)'}}>
                <div style={{fontFamily:rj,fontSize:10,letterSpacing:'2px',textTransform:'uppercase',color:'#c8a84b',marginBottom:8,fontWeight:700}}>Specific marine services (optional)</div>
                <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(190px,1fr))',gap:4}}>
                  {MARINE_SERVICES.map(s=>(<div key={s.key} onClick={()=>toggleMs(s.key)} style={{padding:'6px 10px',border:`1px solid ${ms.has(s.key)?'#c8a84b':'rgba(200,168,75,.2)'}`,background:ms.has(s.key)?'#c8a84b':'transparent',color:ms.has(s.key)?'#08100a':'#b0c0a4',fontFamily:rj,fontSize:11,fontWeight:600,cursor:'pointer',userSelect:'none'}}>{s.label}</div>))}
                </div>
              </div>
            )}
            {done&&(
              <div style={{borderTop:'1px solid rgba(200,168,75,.15)',paddingTop:16,marginTop:16}}>
                ...
                {fb&&results.length>0&&(<div style={{padding:'10px 14px',background:'rgba(200,168,75,.06)',border:'1px solid rgba(200,168,75,.18)',fontSize:12,color:'#e2c06a',marginBottom:8,fontFamily:rj,lineHeight:1.5}}>No providers at <strong>{port}</strong> yet — showing others in <strong>{country}</strong>.</div>)}
                ...
                {results.map(p=>(
                  <div key={p.id} className="rrow" onClick={()=>setDetail(p)} style={{background:'rgba(8,16,10,.7)',border:'1px solid rgba(200,168,75,.2)',padding:'14px 18px',marginBottom:6,display:'grid',gridTemplateColumns:'42px 1fr auto',gap:12,alignItems:'center',transition:'border-color .3s'}}>
                    <div style={{width:42,height:42,background:'rgba(200,168,75,.1)',border:'1px solid rgba(200,168,75,.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18}}>{p.ico}</div>
                    <div>
                      <div style={{fontSize:14,fontWeight:600,marginBottom:2}}>{p.name}</div>
                      <div style={{fontSize:12,color:'#b0c0a4',lineHeight:1.4}}>{p.bio.length>110?p.bio.slice(0,110)+'...':p.bio}</div>
                    </div>
                    <div style={{textAlign:'right',flexShrink:0}}>
                      <div style={{fontFamily:rj,fontSize:9,letterSpacing:'1.5px',textTransform:'uppercase',color:'#7a8a72',marginBottom:5,fontWeight:600}}>{nav2(p.type)}</div>
                      <button onClick={e=>{e.stopPropagation();setDetail(p);}} style={{background:'#c8a84b',border:'none',color:'#08100a',padding:'5px 12px',fontFamily:rj,fontSize:10,letterSpacing:'1px',textTransform:'uppercase',fontWeight:700,cursor:'pointer'}}>View Contact</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* VISUALS */}
        <section className="visuals-pad" style={{background:'#08100a',padding:'60px 56px',textAlign:'center',borderTop:'1px solid rgba(200,168,75,.1)'}}>
          <div style={{display:'flex',justifyContent:'center',marginBottom:36}}>
            <div style={{position:'relative',width:120,height:120}}>
              ...
              <div style={{position:'absolute',inset:12,borderRadius:'50%',border:'1px solid rgba(200,168,75,.2)',animation:'spinSlow 12s linear infinite reverse'}}/>
              <div style={{position:'absolute',inset:24,borderRadius:'50%',border:'1px solid rgba(200,168,75,.15)',animation:'spinSlow 8s linear infinite'}}/>
              <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center'}}>
                <div style={{width:44,height:44,borderRadius:'50%',background:'radial-gradient(circle at 35% 35%,rgba(200,168,75,.4),rgba(200,168,75,.05))',border:'1px solid rgba(200,168,75,.5)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18}}>⚓</div>
              </div>
            </div>
          </div>
          <div style={{overflow:'hidden',borderTop:'1px solid rgba(200,168,75,.1)',borderBottom:'1px solid rgba(200,168,75,.1)',padding:'9px 0',marginBottom:36}}>
            <div style={{display:'flex',gap:36,animation:'scrollBanner 28s linear infinite',whiteSpace:'nowrap'}}>
          ...
          <div className="stats-grid" style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:1,background:'rgba(200,168,75,.12)',maxWidth:860,margin:'0 auto'}}>
            {[['80+','Countries','active providers'],['1,000+','Ports','in database'],['37+','Providers','verified'],['$0','Search Fee','always free']].map(([v,l,s])=>(
              <div key={l} style={{background:'#0c1610',padding:'20px 14px',textAlign:'center'}}>
                <div style={{fontFamily:lb,fontSize:28,fontWeight:700,color:'#c8a84b',lineHeight:1,marginBottom:4}}>{v}</div>
        ...
        {/* HOW IT WORKS */}
        <section id="how" className="section-pad" style={{padding:'100px 56px',background:'#0c1610',borderTop:'1px solid rgba(200,168,75,.12)'}}>
          ...
          <h2 style={{fontFamily:lb,fontSize:'clamp(26px,3.5vw,42px)',fontWeight:700,lineHeight:1.05,marginBottom:44}}>How <em style={g}>PortServiceFinder</em> Works</h2>
          <div className="steps-grid" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:1,background:'rgba(200,168,75,.12)'}}>
            {[{n:'01',ico:'🌍',t:'Select Country & Port',d:'Choose your destination from our global list. Completely free — no account needed.'},
              {n:'02',ico:'🔍',t:'Filter by Service Type',d:'Select Ship Agent, Shipchandler or Marine Services and narrow to specializations.'},
              {n:'03',ico:'📡',t:'Smart Fallback',d:'No provider at your port? We automatically show others in the same country.'}].map(s=>(
              <div key={s.n} className="step" style={{background:'#111c13',padding:'38px 30px',position:'relative',overflow:'hidden',transition:'background .4s'}}>
                <div style={{fontFamily:lb,fontSize:68,fontWeight:700,color:'rgba(200,168,75,.05)',position:'absolute',top:6,right:12,lineHeight:1}}>{s.n}</div>
                <div style={{fontSize:26,marginBottom:14}}>{s.ico}</div>
                <h3 style={{fontFamily:lb,fontSize:19,fontWeight:700,marginBottom:10}}>{s.t}</h3>
                <p style={{fontSize:13,lineHeight:1.8,color:'#b0c0a4'}}>{s.d}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* PRICING */}
        <section id="pricing" className="section-pad" style={{padding:'100px 56px'}}>
          ...
          <h2 style={{fontFamily:lb,fontSize:'clamp(26px,3.5vw,42px)',fontWeight:700,lineHeight:1.05,marginBottom:44}}>Simple, <em style={g}>Transparent</em> Pricing</h2>
          <p style={{color:'#b0c0a4',maxWidth:500,margin:'-30px auto 36px',fontSize:14,lineHeight:1.7,textAlign:'center'}}>No commission. No hidden fees. One flat subscription.</p>
          <div className="tiers-grid" style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:16,maxWidth:720,margin:'0 auto'}}>
            {[{name:'Monthly',amt:'$99',per:'/ month',yr:'Billed monthly · Cancel anytime',badge:null,items:['Listed at all your ports','Full company profile','Phone, email & WhatsApp visible','Verified provider badge','Performance dashboard']},
              {name:'Annual',amt:'$1,000',per:'/ year',yr:'Equivalent to $83/month — save $188',badge:'Save $188',items:['Everything in Monthly','Priority placement in results','$188 saved vs monthly','Verified provider badge','Priority support']}].map(tier=>(
              <div key={tier.name} className="tier" style={{background:tier.badge?'linear-gradient(180deg,rgba(200,168,75,.06),transparent)':'#111c13',border:`1px solid ${tier.badge?'#c8a84b':'rgba(200,168,75,.22)'}`,padding:'32px 26px',position:'relative',transition:'all .4s',display:'flex',flexDirection:'column'}}>
                {tier.badge&&<div style={{position:'absolute',top:-11,left:'50%',transform:'translateX(-50%)',background:'#c8a84b',color:'#08100a',fontFamily:rj,fontSize:10,letterSpacing:'2px',fontWeight:700,padding:'4px 12px'}}>{tier.badge}</div>}
                <div style={{fontFamily:rj,fontSize:11,letterSpacing:'2px',textTransform:'uppercase',color:'#c8a84b',marginBottom:10,fontWeight:700}}>{tier.name}</div>
                <div style={{display:'flex',alignItems:'baseline',gap:5,marginBottom:5}}><span style={{fontFamily:lb,fontSize:42,fontWeight:700,lineHeight:1}}>{tier.amt}</span><span style={{fontFamily:rj,fontSize:12,color:'#7a8a72',fontWeight:600}}>{tier.per}</span></div>
                <div style={{fontSize:12,color:'#b0c0a4',marginBottom:20,fontFamily:rj}}>{tier.yr}</div>
                <ul style={{listStyle:'none',flex:1,marginBottom:20,display:'flex',flexDirection:'column',gap:8}}>
                  {tier.items.map(item=>(<li key={item} style={{fontSize:13,color:'#b0c0a4',display:'flex',alignItems:'flex-start',gap:7,lineHeight:1.5}}><span style={{color:'#c8a84b',fontWeight:700,flexShrink:0}}>✓</span>{item}</li>))}
                </ul>
                <button onClick={()=>{setTab('register');setModal(true);}} style={{padding:12,background:tier.badge?'#c8a84b':'transparent',border:'1px solid rgba(200,168,75,.4)',color:tier.badge?'#08100a':'#c8a84b',fontFamily:rj,fontSize:12,letterSpacing:'2px',textTransform:'uppercase',fontWeight:700,cursor:'pointer',width:'100%'}}>Get Started</button>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* CTA */}
        <section className="cta-pad" style={{padding:'80px 56px',textAlign:'center',background:'#0c1610',borderTop:'1px solid rgba(200,168,75,.12)'}}>
          <h2 style={{fontFamily:lb,fontSize:'clamp(28px,4vw,52px)',fontWeight:700,lineHeight:1.05,marginBottom:14}}>Be Found by Every Vessel <em style={g}>Worldwide</em></h2>
          <p style={{fontSize:15,color:'#b0c0a4',maxWidth:440,margin:'0 auto 32px',lineHeight:1.75}}>List on PortServiceFinder for <strong style={g}>$99/month</strong> or <strong style={g}>$1,000/year</strong>. No setup fee.</p>
          <div style={{display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap'}}>
            <button onClick={()=>{setTab('register');setModal(true);}} style={{background:'#c8a84b',color:'#08100a',border:'none',padding:'13px 32px',fontFamily:rj,fontSize:14,letterSpacing:'2px',textTransform:'uppercase',fontWeight:700,cursor:'pointer'}}>List Your Business</button>
            <button onClick={()=>window.scrollTo({top:0,behavior:'smooth'})} style={{background:'transparent',color:'#f5f0e8',border:'1px solid rgba(200,168,75,.3)',padding:'12px 24px',fontFamily:rj,fontSize:14,letterSpacing:'2px',textTransform:'uppercase',fontWeight:600,cursor:'pointer'}}>Search Free</button>
          </div>
        </section>

        {/* FOOTER */}
        <footer className="footer-pad" style={{borderTop:'1px solid rgba(200,168,75,.18)',padding:'52px 56px 0'}}>
          <div className="footer-grid" style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr',gap:48,marginBottom:36}}>
            <div>
              <div style={{fontFamily:lb,fontSize:20,fontWeight:700,letterSpacing:1,marginBottom:10}}>PortService<span style={g}>Finder</span></div>
              <p style={{fontSize:13,color:'#7a8a72',lineHeight:1.75,maxWidth:220,marginBottom:16}}>The global maritime services directory.</p>
            ...
            {[{t:'Directory',l:['Ship Agents','Shipchandlers','Marine Services','Search by Port']},{t:'Company',l:['About Us','Contact','Blog','Partners']},{t:'Legal',l:['Terms of Service','Privacy Policy','Listing Rules','Disclaimer']}].map(col=>(
              <div key={col.t}>
                <h4 style={{fontFamily:rj,fontSize:10,letterSpacing:'2px',textTransform:'uppercase',color:'#c8a84b',marginBottom:14,fontWeight:700}}>{col.t}</h4>
          ...
          <div style={{borderTop:'1px solid rgba(200,168,75,.12)',padding:'16px 0 24px',display:'flex',justifyContent:'space-between',fontFamily:rj,fontSize:10,color:'#3a3a2a',letterSpacing:1,fontWeight:600,flexWrap:'wrap',gap:8}}>
        ...
        {/* DETAIL MODAL */}
        {detail&&(
          <div style={{position:'fixed',inset:0,background:'rgba(8,16,10,.95)',backdropFilter:'blur(16px)',zIndex:550,display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'40px 16px',overflowY:'auto'}} onClick={e=>{if(e.target===e.currentTarget)setDetail(null);}}>
            <div style={{background:'#0c1610',border:'1px solid rgba(200,168,75,.3)',width:'100%',maxWidth:680,margin:'auto'}}>
              <div style={{padding:'26px 30px 18px',borderBottom:'1px solid rgba(200,168,75,.18)',display:'flex',gap:16,alignItems:'flex-start'}}>
                <div style={{width:54,height:54,background:'rgba(200,168,75,.1)',border:'1px solid rgba(200,168,75,.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:24,flexShrink:0}}>{detail.ico}</div>
                <div style={{flex:1}}>
                  <div style={{fontFamily:lb,fontSize:22,fontWeight:700,marginBottom:3}}>{detail.name}</div>
                  <div style={{fontFamily:rj,fontSize:10,letterSpacing:'2px',textTransform:'uppercase',color:'#c8a84b',fontWeight:700,marginBottom:6}}>{nav2(detail.type)}</div>
                  <span style={{fontFamily:rj,fontSize:10,color:'#4caf76',border:'1px solid rgba(76,175,118,.3)',padding:'2px 8px',letterSpacing:1,fontWeight:700}}>✓ VERIFIED</span>
                </div>
              ...
              <div style={{padding:'22px 30px 28px'}}>
                <p style={{fontSize:13,color:'#f5f0e8',lineHeight:1.7,marginBottom:18}}>{detail.bio}</p>
                <div className="detail-contact" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:18}}>
                  {[{label:'Phone',value:detail.phone,href:`tel:${detail.phone.replace(/\s/g,'')}`},{label:'Email',value:detail.email,href:`mailto:${detail.email}`},{label:'WhatsApp',value:detail.wa,href:`https://wa.me/${detail.wa.replace(/\D/g,'')}`},{label:'Website',value:detail.web.replace(/^https?:\/\//,''),href:detail.web}].map(c=>(
                    <div key={c.label} style={{background:'#111c13',border:'1px solid rgba(200,168,75,.18)',padding:'12px 14px'}}>
                      <div style={{fontFamily:rj,fontSize:9,letterSpacing:'1.5px',textTransform:'uppercase',color:'#7a8a72',marginBottom:4,fontWeight:600}}>{c.label}</div>
                      <a href={c.href} target="_blank" rel="noreferrer" style={{fontSize:13,color:'#c8a84b',textDecoration:'none'}}>{c.value}</a>
                    </div>
                  ))}
                </div>
                <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:16}}>
                  {detail.ports.map(p=><span key={p} style={{fontFamily:rj,fontSize:9,letterSpacing:1,fontWeight:700,padding:'2px 7px',border:'1px solid rgba(200,168,75,.3)',color:'#c8a84b'}}>{p}</span>)}
                </div>
                <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                  <a href={`tel:${detail.phone.replace(/\s/g,'')}`} style={{flex:1,minWidth:120,padding:11,background:'#c8a84b',color:'#08100a',textDecoration:'none',fontFamily:rj,fontSize:11,letterSpacing:'1.5px',textTransform:'uppercase',fontWeight:700,textAlign:'center',display:'flex',alignItems:'center',justifyContent:'center'}}>📞 Call</a>
                  <a href={`mailto:${detail.email}`} style={{flex:1,minWidth:120,padding:11,background:'#c8a84b',color:'#08100a',textDecoration:'none',fontFamily:rj,fontSize:11,letterSpacing:'1.5px',textTransform:'uppercase',fontWeight:700,textAlign:'center',display:'flex',alignItems:'center',justifyContent:'center'}}>✉ Email</a>
                  <a href={`https://wa.me/${detail.wa.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" style={{flex:1,minWidth:120,padding:11,background:'transparent',border:'1px solid rgba(200,168,75,.4)',color:'#c8a84b',textDecoration:'none',fontFamily:rj,fontSize:11,letterSpacing:'1.5px',textTransform:'uppercase',fontWeight:700,textAlign:'center',display:'flex',alignItems:'center',justifyContent:'center'}}>💬 WhatsApp</a>
                </div>
              </div>
            </div>
          </div>
        ...
        {modal&&(
          <div className="modal-pad" style={{position:'fixed',inset:0,background:'rgba(8,16,10,.95)',backdropFilter:'blur(16px)',zIndex:500,display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'40px 16px',overflowY:'auto'}} onClick={e=>{if(e.target===e.currentTarget)setModal(false);}}>
            <div style={{background:'#0c1610',border:'1px solid rgba(200,168,75,.3)',width:'100%',maxWidth:700,margin:'auto'}}>
              <div style={{padding:'22px 30px 16px',borderBottom:'1px solid rgba(200,168,75,.18)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <h2 style={{fontFamily:lb,fontSize:22,fontWeight:700}}>{tab==='login'?'Provider Sign In':'List Your Business'}</h2>
                <button onClick={()=>setModal(false)} style={{background:'none',border:'none',color:'#7a8a72',fontSize:20,cursor:'pointer'}}>✕</button>
              </div>
              <div style={{padding:'22px 30px'}}>
                <div style={{display:'flex',borderBottom:'1px solid rgba(200,168,75,.18)',marginBottom:20}}>
                  {(['register','login'] as const).map(t=>(<button key={t} onClick={()=>setTab(t)} style={{padding:'9px 18px',fontFamily:rj,fontSize:11,letterSpacing:'1.5px',textTransform:'uppercase',fontWeight:700,cursor:'pointer',color:tab===t?'#c8a84b':'#7a8a72',background:'none',border:'none',borderBottom:tab===t?'2px solid #c8a84b':'2px solid transparent',marginBottom:-1}}>{t==='register'?'Register':'Sign In'}</button>))}
                </div>
                {tab==='register'?(
                  <div>
                    <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:18}}>
                      {[{type:'agent',ico:'🏢',name:'Ship Agent'},{type:'chandler',ico:'⚓',name:'Shipchandler'},{type:'service',ico:'🔧',name:'Marine Service'}].map(s=>(
                        <div key={s.type} onClick={()=>setSeg(s.type)} style={{border:`1px solid ${seg===s.type?'#c8a84b':'rgba(200,168,75,.2)'}`,padding:'12px 8px',textAlign:'center',cursor:'pointer',background:seg===s.type?'rgba(200,168,75,.1)':'transparent'}}>
                          <div style={{fontSize:18,marginBottom:4}}>{s.ico}</div>
                          <div style={{fontFamily:rj,fontSize:11,letterSpacing:1,textTransform:'uppercase',fontWeight:700}}>{s.name}</div>
                          <div style={{fontSize:10,color:'#c8a84b',marginTop:2,fontFamily:rj}}>$99/mo</div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}><FI l="Email" p="info@company.com" t="email"/><FI l="Phone" p="+1 ..."/></div>
                    <FI l="Contact Person" p="Primary contact name"/>
                    <div style={{marginBottom:14}}>
                      <label style={{display:'block',fontFamily:rj,fontSize:10,letterSpacing:'1.5px',textTransform:'uppercase',color:'#7a8a72',marginBottom:4,fontWeight:600}}>Company Bio</label>
                      <textarea maxLength={500} placeholder="Brief description..." style={{background:'rgba(8,16,10,.7)',border:'1px solid rgba(200,168,75,.25)',color:'#f5f0e8',padding:'9px 12px',fontSize:13,width:'100%',resize:'vertical',minHeight:70}}/>
                    </div>
                    ...
                    <button onClick={()=>{setModal(false);setPayModal(true);}} style={{width:'100%',padding:13,background:'#c8a84b',border:'none',color:'#08100a',fontFamily:rj,fontSize:13,letterSpacing:'2px',textTransform:'uppercase',fontWeight:700,cursor:'pointer',marginTop:6}}>Continue to Payment</button>
                    <p style={{fontSize:10,color:'#7a8a72',textAlign:'center',marginTop:8}}>$99/month or $1,000/year. Cancel anytime.</p>
                  </div>
                ):(
                  <div>
                    <FI l="Email" p="your@company.com" t="email"/>
                    <FI l="Password" p="password" t="password"/>
                    <button style={{width:'100%',padding:13,background:'#c8a84b',border:'none',color:'#08100a',fontFamily:rj,fontSize:13,letterSpacing:'2px',textTransform:'uppercase',fontWeight:700,cursor:'pointer',marginTop:6}}>Sign In</button>
                    <p style={{textAlign:'center',fontSize:12,color:'#7a8a72',marginTop:12}}>Not registered? <span style={{color:'#c8a84b',cursor:'pointer'}} onClick={()=>setTab('register')}>List your business →</span></p>
                  </div>
                )}
              </div>
            </div>
        ...
        {payModal&&(
          <div style={{position:'fixed',inset:0,background:'rgba(8,16,10,.96)',backdropFilter:'blur(20px)',zIndex:600,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px',overflowY:'auto'}} onClick={e=>{if(e.target===e.currentTarget)setPayModal(false);}}>
            <div style={{background:'#0c1610',border:'1px solid rgba(200,168,75,.3)',width:'100%',maxWidth:580,padding:36,margin:'auto',position:'relative'}}>
              <button onClick={()=>setPayModal(false)} style={{position:'absolute',top:16,right:16,background:'none',border:'none',color:'#7a8a72',fontSize:20,cursor:'pointer'}}>✕</button>
              <h2 style={{fontFamily:lb,fontSize:24,fontWeight:700,marginBottom:4}}>Choose Your <em style={g}>Plan</em></h2>
              <p style={{fontSize:12,color:'#b0c0a4',marginBottom:20,lineHeight:1.6}}>Cancel anytime. No setup fee.</p>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:22}}>
                {[{id:'monthly',label:'Monthly',price:'$99',period:'per month',note:'Billed monthly.',badge:null},{id:'yearly',label:'Annual',price:'$1,000',period:'per year',note:'$83/month — save $188.',badge:'Save $188'}].map(p=>(
                  <div key={p.id} onClick={()=>setPlan(p.id as 'monthly'|'yearly')} style={{border:`2px solid ${plan===p.id?'#c8a84b':'rgba(200,168,75,.22)'}`,padding:'22px 18px',cursor:'pointer',position:'relative',background:plan===p.id?'rgba(200,168,75,.07)':'transparent'}}>
                    {p.badge&&<div style={{position:'absolute',top:12,right:12,background:'#c8a84b',color:'#08100a',fontFamily:rj,fontSize:9,letterSpacing:'1.5px',fontWeight:700,padding:'3px 7px'}}>{p.badge}</div>}
                    <div style={{fontFamily:rj,fontSize:10,letterSpacing:'2px',textTransform:'uppercase',color:'#c8a84b',fontWeight:700,marginBottom:8}}>{p.label}</div>
                    <div style={{fontFamily:lb,fontSize:32,fontWeight:700,lineHeight:1}}>{p.price}</div>
                    <div style={{fontFamily:rj,fontSize:11,color:'#7a8a72',fontWeight:600,marginTop:3}}>{p.period}</div>
                    <div style={{fontSize:11,color:'#b0c0a4',marginTop:8}}>{p.note}</div>
                  </div>
                ))}
              </div>
              <FI l="Cardholder Name" p="Name on card"/>
              <FI l="Card Number" p="1234 5678 9012 3456"/>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><FI l="Expiry" p="MM/YY"/><FI l="CVC" p="123"/></div>
              <button onClick={()=>{setPayModal(false);alert('Welcome to PortServiceFinder! Your listing is now active.');}} style={{width:'100%',padding:13,background:'#c8a84b',border:'none',color:'#08100a',fontFamily:rj,fontSize:13,letterSpacing:'2px',textTransform:'uppercase',fontWeight:700,cursor:'pointer',marginTop:16}}>
                Pay {plan==='yearly'?'$1,000':'$99'} & Activate
              </button>
              <div style={{fontFamily:rj,fontSize:10,color:'#7a8a72',textAlign:'center',marginTop:10}}>🔒 Secure payment · Cancel anytime</div>
            </div>
          </div>
        )}
...
function FI({l,p,t='text'}:{l:string;p:string;t?:string}) {
  return (
    <div style={{marginBottom:10}}>
      <label style={{display:'block',fontFamily:"'Rajdhani',sans-serif",fontSize:10,letterSpacing:'1.5px',textTransform:'uppercase',color:'#7a8a72',marginBottom:4,fontWeight:600}}>{l}</label>
      <input type={t} placeholder={p} style={{background:'rgba(8,16,10,.7)',border:'1px solid rgba(200,168,75,.25)',color:'#f5f0e8',padding:'9px 12px',fontSize:13,width:'100%'}}/>
    </div>
  );
}
